...
/// Data table for Manager Tasks displaying all task specifications, assignments, PR status, and action triggers.
class ManagerTasksTable extends StatelessWidget {
  const ManagerTasksTable({super.key, required this.tasks});
          ...
          rows: tasks.map((task) {
            ...
            return DataRow(
              cells: [
                ...
                DataCell(
                  Text(task.assignedTo.isNotEmpty ? task.assignedTo : 'Unassigned', style: const TextStyle(color: AppColors.textSecondary, fontSize: 13)),
                ),
                DataCell(
                  Text(task.githubRepo ?? 'VALIXIS_PORTAL', style: const TextStyle(color: AppColors.brandBlue, fontSize: 12, fontWeight: FontWeight.w600)),
                ),
                DataCell(
                ...
                DataCell(
                  Text(DateFormatter.formatShortDate(task.deadline), style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                ),
                DataCell(
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(
                      color: (hasPr ? AppColors.success : AppColors.textMuted).withAlpha(20),
                      borderRadius: BorderRadius.circular(6),
                    ),
                    child: Text(
                      hasPr ? 'Yes' : 'No',
                      style: TextStyle(color: hasPr ? AppColors.success : AppColors.textMuted, fontSize: 11, fontWeight: FontWeight.w700),
                    ),
                  ),
                ),
                DataCell(
                  Row(
                    children: [
                      ...
                      IconButton(
                        icon: const Icon(Icons.archive_outlined, size: 18, color: AppColors.textMuted),
